...
class basic_sparse_set {
    ...
    [[nodiscard]] static auto page(const Entity entt) ENTT_NOEXCEPT {
        return size_type{entity_traits::to_entity(entt) / sparse_page};
    }

    [[nodiscard]] static auto offset(const Entity entt) ENTT_NOEXCEPT {
        return size_type{entity_traits::to_entity(entt) & (sparse_page - 1)};
    }
...
protected:
    ...
    virtual void swap_and_pop(const Entity entt, [[maybe_unused]] void *ud) {
        ...
        const auto pos = size_type{entity_traits::to_entity(ref)};
...
public:
    ...
    [[nodiscard]] size_type slot() const ENTT_NOEXCEPT {
        return free_list == null ? count : size_type{entity_traits::to_entity(free_list)};
    }
    ...
    [[nodiscard]] size_type index(const entity_type entt) const ENTT_NOEXCEPT {
        ...
        return size_type{entity_traits::to_entity(sparse[page(entt)][offset(entt)])};
    }
    ...
    size_type emplace(const entity_type entt) {
        if(free_list == null) {
            ...
            ENTT_ASSERT(!contains(entt), "Set already contains entity");
            const auto pos = size_type{entity_traits::to_entity(free_list)};
            sparse[page(entt)][offset(entt)] = entity_traits::construct(static_cast<typename entity_traits::entity_type>(pos));
    ...
    void swap(const entity_type lhs, const entity_type rhs) {
        ...
        const auto from = size_type{entity_traits::to_entity(entt)};
        const auto to = size_type{entity_traits::to_entity(other)};
